...
class TkLightSensor(TkDevice):
    def __init__(self, root, x, y, name, pin):
        ...
        self._scale = Scale(root, from_=0, to=90, showvalue=0,
                            orient=VERTICAL, command=self._scale_changed, sliderlength=20, length=150,
                            highlightthickness=0, background="white")
    ...
    def __init__(self, root, x, y, name, pin):
        """
                ...
                name : Str
                    Name of the motor
                pins : List
                    2 pins associated with motor
        """
        super().__init__(root, x, y, name)

        if pin[0] is None or pin[1] is None:
           print("Not a valid pin configuration for motor.")

        self.loop = False
        self._pin0 = Device.pin_factory.pin(pin[0])
        self._pin1 = Device.pin_factory.pin(pin[1])

        self._previous_state = None
        self._previous_state1 = None

        TkMotor.on_image = self._set_image_for_state("motor_forward.png", "on_forward", (150, 100))
        TkMotor.on_image = self._set_image_for_state("motor_backwards.png", "on_backwards", (150, 100))
        self._set_image_for_state("motor_off.png", "off", (150, 100))
        self._create_main_widget(Label, "off")

    def update(self):
        # print("PIN0: " + str(self._pin0.state))
        # print("PIN1: " + str(self._pin1.state))
        if self._previous_state != self._pin0.state or self._previous_state1 != self._pin1.state:
            if self._pin0.state > 0:
                self._change_widget_image("on_forward")
            elif self._pin1.state > 0:
                self._change_widget_image("on_backwards")
            else:
                self._change_widget_image("off")

            self._previous_state = self._pin0.state
            self._previous_state1 = self._pin1.state
